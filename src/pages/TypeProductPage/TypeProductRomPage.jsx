...
const TypeProductRomPage = () => {
    ...
    const fetchRomProducts = async (search) => {
    ...
    useEffect(() => {
        if (refSearch.current) {
            setLoading(true);
            fetchRomProducts(searchProduct);
        }
    ...
    const { isLoading, data: products } = useQuery({
        queryKey: ['romProducts'],
        queryFn: () => fetchRomProducts(searchProduct),
        retry: 3,
        retryDelay: 1000
    });
            ...
            <div className="container">
                ...
                <div>
                    <div className="WrapperProduct">
                        {getPaginatedProducts(stateProducts.filter((product) => product.type === 'rom'), currentPage, pageSize)
                            .map((product) => {
                    ...
                    <div className="WrapperPagination">
                        <Pagination current={currentPage} total={stateProducts.filter((product) => product.type === 'rom').length} pageSize={pageSize} onChange={onChange} />
                    </div>
                </div>
            </div>
        </Loading>
...
export default TypeProductRomPage;
