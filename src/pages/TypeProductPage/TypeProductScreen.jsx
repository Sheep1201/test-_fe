...
const TypeProductScreenPage = () => {
    ...
    const fetchScreenProducts = async (search) => {
    ...
    useEffect(() => {
        if (refSearch.current) {
            setLoading(true);
            fetchScreenProducts(searchProduct);
        }
    ...
    const { isLoading, data: products } = useQuery({
        queryKey: ['screenProducts'],
        queryFn: () => fetchScreenProducts(searchProduct),
        retry: 3,
        retryDelay: 1000
    });
            ...
            <div className="container">
                ...
                <div>
                    <div className="WrapperProduct">
                        {getPaginatedProducts(stateProducts.filter((product) => product.type === 'screen'), currentPage, pageSize)
                            .map((product) => {
                    ...
                    <div className="WrapperPagination">
                        <Pagination current={currentPage} total={stateProducts.filter((product) => product.type === 'screen').length} pageSize={pageSize} onChange={onChange} />
                    </div>
                </div>
            </div>
        </Loading>
...
export default TypeProductScreenPage;
