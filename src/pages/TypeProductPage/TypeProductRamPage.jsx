...
const TypeProductRamPage = () => {
    ...
    const fetchRamProducts = async (search) => {
    ...
    useEffect(() => {
        if (refSearch.current) {
            setLoading(true);
            fetchRamProducts(searchProduct);
        }
    ...
    const { isLoading, data: products } = useQuery({
        queryKey: ['ramProducts'],
        queryFn: () => fetchRamProducts(searchProduct),
        retry: 3,
        retryDelay: 1000
    });
            ...
            <div className="container">
                ...
                <div>
                    <div className="WrapperProduct">
                        {getPaginatedProducts(stateProducts.filter((product) => product.type === 'ram'), currentPage, pageSize)
                            .map((product) => {
                    ...
                    <div className="WrapperPagination">
                        <Pagination current={currentPage} total={stateProducts.filter((product) => product.type === 'ram').length} pageSize={pageSize} onChange={onChange} />
                    </div>
                </div>
            </div>
        </Loading>
...
export default TypeProductRamPage;
