...
const TypeProductKeyboardPage = () => {
    ...
    const fetchKeyboardProducts = async (search) => {
    ...
    useEffect(() => {
        if (refSearch.current) {
            setLoading(true);
            fetchKeyboardProducts(searchProduct);
        }
    ...
    const { isLoading, data: products } = useQuery({
        queryKey: ['keyboardProducts'],
        queryFn: () => fetchKeyboardProducts(searchProduct),
        retry: 3,
        retryDelay: 1000
    });
            ...
            <div className="container">
                ...
                <div>
                    <div className="WrapperProduct">
                        {getPaginatedProducts(stateProducts.filter((product) => product.type === 'keyboard'), currentPage, pageSize)
                            .map((product) => {
                    ...
                    <div className="WrapperPagination">
                        <Pagination current={currentPage} total={stateProducts.filter((product) => product.type === 'keyboard').length} pageSize={pageSize} onChange={onChange} />
                    </div>
                </div>
            </div>
        </Loading>
...
export default TypeProductKeyboardPage;
