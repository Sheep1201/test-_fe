...
const TypeProductGearPage = () => {
    ...
    const fetchGearProducts = async (search) => {
    ...
    useEffect(() => {
        if (refSearch.current) {
            setLoading(true);
            fetchGearProducts(searchProduct);
        }
    ...
    const { isLoading, data: products } = useQuery({
        queryKey: ['gearProducts'],
        queryFn: () => fetchGearProducts(searchProduct),
        retry: 3,
        retryDelay: 1000
    });
            ...
            <div className="container">
                ...
                <div>
                    <div className="WrapperProduct">
                        {getPaginatedProducts(stateProducts.filter((product) => product.type === 'gear'), currentPage, pageSize)
                            .map((product) => {
                    ...
                    <div className="WrapperPagination">
                        <Pagination current={currentPage} total={stateProducts.filter((product) => product.type === 'gear').length} pageSize={pageSize} onChange={onChange} />
                    </div>
                </div>
            </div>
        </Loading>
...
export default TypeProductGearPage;
